...
static int32 encodeLowFreq(float lowFreq) {
	float lf = std::clamp(lowFreq, 40.875885f, 626.286133f);
	return (int32)std::round(32 * std::log2(lf * 0.1)) - 0x40;
}
static int32 encodeHighFreq(float highFreq) {
	float hf = std::clamp(highFreq, 81.75177f, 1252.572266f);
	return (int32)(std::round(32 * std::log2(hf * 0.1)) - 0x60) * 4;
}
static int32 encodeLowAmpli(float rawAmpli) {
	int32 encodedAmpli = 0;
	if (0 < rawAmpli && rawAmpli < 0.012f) {
		encodedAmpli = 1;
	}
	else if (0.012f <= rawAmpli && rawAmpli < 0.112f) {
		encodedAmpli = (int32)std::round(4 * std::log2(rawAmpli * 110));
	}
	else if (0.112f <= rawAmpli && rawAmpli < 0.225f) {
		encodedAmpli = (int32)std::round(16 * std::log2(rawAmpli * 17));
	}
	else if (0.225f <= rawAmpli && rawAmpli <= 1.0f) {
		encodedAmpli = (int32)std::round(32 * std::log2(rawAmpli * 8.7));
	}
	return (int32)std::floor(encodedAmpli / 2) + 64;
}
static int32 encodeHighAmpli(float rawAmpli) {
	int32 encodedAmpli = 0;
	if (0 < rawAmpli && rawAmpli < 0.012f) {
		encodedAmpli = 1;
	}
...
void UJoyShockLibrary::JslSetHDRumble(int32 deviceId, float lowFreq, float lowAmpli, float highFreq, float highAmpli)
{
	FJoyShockLibrary4UnrealModule& JSL4UModule = FJoyShockLibrary4UnrealModule::GetInstance();
	...
	if (jc != nullptr && jc->controller_type == ControllerType::n_switch) {
		jc->modifying_lock.Lock();
		int32 lf = encodeLowFreq(lowFreq);
		int32 la = encodeLowAmpli(lowAmpli);
		int32 hf = encodeHighFreq(highFreq);
		int32 ha = encodeHighAmpli(highAmpli);

		jc->small_rumble = (int32)(lowAmpli * 255);
		jc->big_rumble = (int32)(highAmpli * 255);
...
void UJoyShockLibrary::JslSetHDRumbleLR(int32 deviceId, float lowFreq_L, float lowAmpli_L, float highFreq_L, float highAmpli_L,
	float lowFreq_R, float lowAmpli_R, float highFreq_R, float highAmpli_R)
{
	...
	if (jc != nullptr && jc->controller_type == ControllerType::n_switch) {
		jc->modifying_lock.Lock();
		jc->small_rumble = (int32)((lowAmpli_L + lowAmpli_R) / 2 * 255);
		jc->big_rumble = (int32)((highAmpli_L + highAmpli_R) / 2 * 255);
		jc->set_joycon_rumble(
